//+------------------------------------------------------------------+
//|  TradeOps.mqh                                                     |
//|  Thin wrapper over CTrade with normalization helpers              |
//+------------------------------------------------------------------+
...
public:
   ...
   bool              Init(const string symbol,
                          ...
                          CLogger     *logger)
     {
      ...
      if(!m_sym.Name(symbol))
        {
         if(m_log) m_log.Error("CSymbolInfo init failed for " + symbol);
         return false;
        }
      m_sym.RefreshRates();
   ...
   //--- Volume normalization to symbol step / min / max
   ...
   //--- Open market order; returns ticket or 0 on failure
   ulong             OpenMarket(const ENUM_ORDER_TYPE type,
                                ...
                                const string          comment)
     {
      ...
      if(v <= 0.0)
        {
         if(m_log) m_log.Warn("OpenMarket: normalized volume = 0");
         return 0;
        }
      ...
      if(type == ORDER_TYPE_BUY)
        ...
        {
         if(m_log) m_log.Error("OpenMarket: unsupported type");
         return 0;
        }
      ...
      if(!ok)
        {
         if(m_log)
            m_log.Error(StringFormat("OpenMarket failed retcode=%u err=%d",
   ...
   bool              ClosePosition(const ulong ticket)
     {
      ...
      if(!ok && m_log)
         m_log.Error(StringFormat("Close failed ticket=%I64u retcode=%u",
                                  ticket, m_trade.ResultRetcode()));
      return ok;
     }

   //--- Partially close; pct in [1..100]
   bool              PartialClose(const ulong ticket, const double pct)
     {
      ...
      if(!ok && m_log)
         m_log.Error(StringFormat("PartialClose failed ticket=%I64u retcode=%u",
                                  ticket, m_trade.ResultRetcode()));
      return ok;
     }
  };
